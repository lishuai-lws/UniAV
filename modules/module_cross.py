...
class CrossEmbeddings(nn.Module):
    ...
    def forward(self, q_input, kv_input, concat_embeddings=None, concat_type=None):
        ...
        #TODO:不需要位置和模态embedding吧
        # embeddings = concat_embeddings
        # embeddings = self.LayerNorm(embeddings)
        # embeddings = self.dropout(embeddings)
        audio_input_embeddings = q_input
        audio_input_embeddings = self.LayerNorm(audio_input_embeddings)
        audio_input_embeddings = self.dropout(audio_input_embeddings)
        visual_input_embeddings = kv_input
        visual_input_embeddings = self.LayerNorm(visual_input_embeddings)
        visual_input_embeddings = self.dropout(visual_input_embeddings)
        return audio_input_embeddings, visual_input_embeddings
...
class CrossSelfAttention(nn.Module):
    ...
    def forward(self, q_hidden_states, kv_hidden_states, attention_mask):
...
class CrossAttention(nn.Module):
    ...
    def forward(self, q_hidden_states, kv_hidden_states, attention_mask):
        self_output = self.self(q_hidden_states, kv_hidden_states, attention_mask)
        attention_output = self.output(self_output, q_hidden_states)
...
class CrossLayer(nn.Module):
    ...
    def forward(self, q_hidden_states, kv_hidden_states, attention_mask):
        attention_output = self.attention(q_hidden_states, kv_hidden_states, attention_mask)
...
class CrossEncoder(nn.Module):
    ...
    def forward(self, q_hidden_states, kv_hidden_states, attention_mask, output_all_encoded_layers=True):
        all_encoder_layers = []
        for layer_module in self.layer:
            hidden_states = layer_module(q_hidden_states, kv_hidden_states, attention_mask)
...
class CrossModel(PreTrainedModel):
    ...
    def forward(self, q_input, kv_input,  attention_mask=None, output_all_encoded_layers=True):

        if attention_mask is None:
            attention_mask = torch.ones(q_input.size(0)+kv_input.size(0), q_input.size(1)+kv_input.size(1))
        ...
        audio_embedding_output, visual_embedding_output = self.embeddings(q_input, kv_input)
        encoded_layers = self.encoder(audio_embedding_output, visual_embedding_output,
...
class AVEncoder(PreTrainedModel):
    def __init__(self, audio_model, visual_model, cross_model):
