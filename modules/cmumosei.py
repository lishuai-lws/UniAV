...
from UniAV.feature_embedding.module_embedding import AudioWav2Vec2, Resnet50
import argparse
import librosa
import os
import pandas as pd
...
def get_args(description='data embedding'):
    ...
    parser.add_argument("--cmumosei_embedded_video_json_path", default= "/home/lishuai/workspace/output/cmumosei_embedded_video.json")
...
def cmumosei_video_data_embedding(opts):
    ids_path = opts.cmumosei_ids_path
    audios_path = opts.cmumosei_audio_path
    videos_path = opts.cmumosei_video_path
    ids = np.array(pd.read_csv(ids_path))
    ids = ids.reshape(ids.shape[0], ).tolist()
    json_path = opts.cmumosei_embedded_video_json_path
    check_json = load_json(json_path)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    resnet50 = Resnet50(opts).to(device)

    for id in tqdm(ids):
        print("id:", id)  # DNzA2UIkRZk_2 4142
        if id in check_json:
            print("video", id, "embedded")
            continue
        audio_path = os.path.join(audios_path, id + ".wav")
        # read audio
        audio_second = librosa.get_duration(filename=audio_path)
        # read video images
        video_path = os.path.join(videos_path, id + "_aligned")
